var intercomFacade = (function(settings) {
...
var loadFakeIntercom = function () {
    if (document.querySelector("#intercom-fake-btn")) return;
    ...
    region.addEventListener('click', function() {
        if (!document.querySelector(".swal2-popup.cookieBot") && !document.querySelector(".show--consent .cm")) {
            Swal.fire({
...
function handleIntercom() {
    if (CookieConsent.acceptedCategory('preferences')) {
        if (document.querySelector("#intercom-fake-btn")) {
            document.querySelector("#intercom-fake-btn").remove();
        }
        intercomFacade(window.intercomSettings);
    } else {
        loadFakeIntercom();
    }
}

window.addEventListener('cc:onConsent', handleIntercom);
window.addEventListener('cc:onChange', handleIntercom);